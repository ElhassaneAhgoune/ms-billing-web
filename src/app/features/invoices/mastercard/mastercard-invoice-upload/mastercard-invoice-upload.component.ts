...
export class MastercardInvoiceUploadComponent {
  selectedFiles: File[] = [];
  isUploading = false;
  uploadProgress = 0;
  uploadSuccess = false;
    ...
    if (this.uploadMode === 'single' && this.selectedFiles.length === 1) {
      ...
      this.csvUploadService.uploadMastercardFile(this.selectedFiles[0]).subscribe({
        next: (response) => {
          ...
          this.handleUploadSuccess('Mastercard invoice file uploaded successfully.');
        },
        error: (error) => {
      ...
      this.csvUploadService.uploadMultipleMastercardFiles(this.selectedFiles).subscribe({
        next: (response) => {
          clearInterval(progressInterval);
          this.handleUploadSuccess(`${this.selectedFiles.length} Mastercard invoice files uploaded successfully.`);
        },
        error: (error) => {
          console.error('CSV upload service error:', error);
          // Fallback to billing service if CSV upload service fails
